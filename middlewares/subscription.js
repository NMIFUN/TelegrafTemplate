...
module.exports = async (ctx, next) => {
  ...
  if(channels?.length) notSubscribed = await asyncFilter(channels, async channel => {
  ...
  if(bots?.length) notSubscribed = notSubscribed.concat(await asyncFilter(bots, async bot => {
    ...
    return ctx[ctx.message || !ctx.callbackQuery.message.text ? 'replyWithHTML' : 'editMessageText'](ctx.i18n.t('subscribe.text'), {
      ...
      reply_markup: Markup.inlineKeyboard([
        ...notSubscribed.map((channel, index) => {
          return Markup.urlButton(`${ctx.i18n.t(`subscribe.${channel.token ? 'bot' : 'channel'}`)} №${index + 1}`, channel.link)
        }),
