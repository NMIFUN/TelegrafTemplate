...
module.exports = async (ctx) => {
  ...
  if(!config.botStat) config.botStat = {
    send: false,
    alive: false
  }
  ...
  if(['alive','send'].includes(ctx.state[0])) {
    ...
    return ctx.editMessageText(`Введите ключ.`, Markup.inlineKeyboard([
...
Текущий ключ: ${config.botStat.key || 'нет'} (<a href='https://botstat.io/dashboard/api'>получение ключа</a>)`, 
  Markup.inlineKeyboard([
    [
      Markup.callbackButton(`Отправлять ${config.botStat.send ? '✅' : '❌'}`, 'admin_botStat_send'),
      Markup.callbackButton(`Живые ${config.botStat.alive ? '✅' : '❌'}`, 'admin_botStat_alive'),
