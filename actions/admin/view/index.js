...
module.exports = async (ctx) => {
  ...
  if(count === 0) return ctx.editMessageText(`Нет просмотров`, {
    ...
    if(ctx.state[1]) {
      ...
      result.status = Object.keys(statuses)[pureIndex >= 0 ? pureIndex : length + pureIndex]
      await result.save()
    }
    ...
    let extraKeyboard = [
      ...
      [
        Markup.callbackButton(`👉${statuses[result.status]}👈`, `admin_view_id_${a}_${result._id}`),
      ],
    ] 
    ...
    if(result.status === 'notStarted') extraKeyboard = extraKeyboard.concat([
      ...
      [ 
        Markup.callbackButton(`🕤 Окончание ${result.endDate ? new Date(result.endDate).toLocaleString('ru', dateConfig) : '❌'}`, `admin_view_endDate_${result._id}_end`),
        Markup.callbackButton(`🧹`, `admin_view_endDate_${result._id}_del`),
      ],
      [ 
        Markup.callbackButton(`🫂 Макс кол-во ${result.quantity === 0 ? '♾️' : result.quantity}`, `admin_view_quantity_${result._id}`),
        Markup.callbackButton(`🧹`, `admin_view_quantity_${result._id}_del`),
      ],
      [ 
        Markup.callbackButton(`🏳️ Язык ${result.lang === null ? 'все' : result.lang }`, `admin_view_lang_${result._id}`),
        Markup.callbackButton(`🧹`, `admin_view_lang_${result._id}_del`),
      ],
      [ 
        Markup.callbackButton(`🌐 Превью ${result.preview ? '✅' : '❌' }`, `admin_view_preview_${result._id}`),
        Markup.callbackButton(`✉️ Уникальные ${result.unique ? '✅' : '❌'}`, `admin_view_unique_${result._id}`),
        Markup.callbackButton(`📃 Изменить пост`, `admin_view_editPost_${result._id}`),
      ]
    ])
      
    if(['doing','ended'].includes(result.status)) extraKeyboard = extraKeyboard.concat([
      [
        Markup.callbackButton(`👁 Просмотров ${result.views}`, `admin_view_none`),
      ]
    ])
