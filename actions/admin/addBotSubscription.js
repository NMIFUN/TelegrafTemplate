...
module.exports = async (ctx) => {
  if(ctx.updateType === 'callback_query') {
    ...
    return ctx.editMessageText(`Для добавления бота на обязательную подписку введите его токен и ссылку (и код языка если нужно) через пробел\nПример: 
...
Текущий список каналов/чатов на обязательную подписку: ${config.subsBots?.map(e => `<a href='${e.link}'>${e.id}</a> ${e.lang} (<code>${e.id}</code>)`).join(", ")}`, { 
      ...admin.backKeyboard,
      parse_mode: "HTML",
    ...
    if(find !== -1) config.subsBots.splice(find, 1)
      ...
      if(!list[1]) return ctx.replyWithHTML(`Не указана ссылка на бот.`, admin.backKeyboard)
      
      find = config.subsBots.findIndex(o => o.id === id)
      if(find === -1) config.subsBots.push({
        link: list[1],
        id: id,
        lang: list[2] || 'all',
        token: list[0]
    ...
    return ctx.replyWithHTML(`Список ботов на обязательную подписку обновлен.\n
Текущий список: ${config.subsBots.map(e => `<a href='${e.link}'>${e.id}</a> ${e.lang} (<code>${e.id}</code>)`).join(", ")}`, { 
      ...admin.backKeyboard,
      disable_web_page_preview: true
    })
  }
