  ...
  if(ctx.state[0]) {
    ...
    switch (ctx.state[0]) {
      ...
      case 'full':
        ...
        contentUsers.push(users.map((value) => Object.values({ ...exportTemplate, ...value }).join(';')))
        break
    }

    return ctx.replyWithDocument({ source: Buffer.from(contentUsers.join('\n'), 'utf8'), filename: `users.csv` })
  } else {
