...
module.exports = async (ctx) => {
  if(ctx.updateType === 'callback_query') {
    ...
    if(ctx.state[1]) {  
      const mail = await ctx.Mail.findByIdAndUpdate(ctx.state[0], { $unset: { startDate: 1 }})
    ...
    try {
      var mail = await ctx.Mail.findByIdAndUpdate(ctx.state[0], { startDate: new Date(ctx.message.text) })
    } catch (error) {
